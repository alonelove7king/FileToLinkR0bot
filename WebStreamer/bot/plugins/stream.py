...
async def private_receive_handler(c: Client, m: Message):
    ...
    if Var.UPDATES_CHANNEL != "None":
        try:
            ...
            await c.send_message(
                chat_id=m.chat.id,
                text="**Sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ Wʀᴏɴɢ. Cᴏɴᴛᴀᴄᴛ ᴍe** @YeYavar",
                parse_mode="markdown",
                disable_web_page_preview=True)
            return
    try:
...
async def channel_receive_handler(bot, broadcast):
    ...
    try:
        ...
        await log_msg.reply_text(
            text=f"**Cʜᴀɴɴᴇʟ Nᴀᴍᴇ:** `{broadcast.chat.title}`\n**Cʜᴀɴɴᴇʟ ID:** `{broadcast.chat.id}`\n**Rᴇǫᴜᴇsᴛ ᴜʀʟ:** https://t.me/{(await bot.get_me()).username}?start=YeYavar_{str(log_msg.message_id)}",
        ...
        await bot.edit_message_reply_markup(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("Dᴏᴡɴʟᴏᴀᴅ ʟɪɴᴋ 📥", url=f"https://t.me/{(await bot.get_me()).username}?start=YeYavar_{str(log_msg.message_id)}")]])
